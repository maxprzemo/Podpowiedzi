...
# adb shell am start -n $pkg/$act
	Linia=3
	T67=$(cat Wynik.txt | awk "{ sum += $Linia } END { print sum }")
	cat Wynik.txt | wc -l > Wiersze.txt
	T68=$(cat Wiersze.txt)
	T69=$(echo "scale = 2; ${T67} / ${T68}" | bc | awk '{printf "%.2f \n", $1}')
	echo "                               Średnio   | ${T69} |" >> Wynik.txt
